 ...
 */
import java.util.*;
class DonHang47 implements Comparable<DonHang47>{
    private String maDH, tenDH;
    private long tienck, thanhtien;

    public DonHang47(String maDH, String tenDH, long sl, long dongia) {
        this.maDH = maDH;
        this.tenDH = tenDH;
    ...
    public int compareTo(DonHang47 other){
        return Long.compare(other.tienck, this.tienck);
    }
}
public class J05047 {
    public static void main(String[] args) {
        ...
        int n = sc.nextInt();
        ArrayList<DonHang47> arr = new ArrayList<>();
        ...
        for(int i=0;i<n;i++){
            ...
            if(mp.containsKey(ma)){
                arr.add(new DonHang47(String.format(ma+"%02d", mp.get(ma)+1),ten,sl,dongia));
                mp.put(ma, mp.get(ma)+1);
            }
            else {
                arr.add(new DonHang47(String.format(ma+"%02d", 1),ten,sl,dongia));
                mp.put(ma, 1);
            }
        }
